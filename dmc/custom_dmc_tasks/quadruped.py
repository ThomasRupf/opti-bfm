...
def make_model(floor_size=None, terrain: bool = False, rangefinders: bool = False, walls_and_ball: bool = False):
...
class Physics(mujoco.Physics):
    ...
    def egocentric_state(self) -> Any:
        ...
        if not self._hinge_names:
            ...
            self._hinge_names = [self.model.id2name(j_id, 'joint') for j_id in hinge_ids]
        return np.hstack(
            (self.named.data.qpos[self._hinge_names], self.named.data.qvel[self._hinge_names], self.data.act)
        )
    ...
    def force_torque(self) -> Any:
        ...
        force_torque_sensors = self._get_sensor_names(enums.mjtSensor.mjSENS_FORCE, enums.mjtSensor.mjSENS_TORQUE)
    ...
    def ball_to_target_distance(self) -> Any:
        ...
        ball_to_target = self.named.data.site_xpos['target'] - self.named.data.xpos['ball']
    ...
    def initialize_episode(self, physics) -> None:
        ...
        if physics.contexts:
            with physics.contexts.gl.make_current() as ctx:
                ctx.call(mjlib.mjr_uploadHField, physics.model.ptr, physics.contexts.mujoco.ptr, _HEIGHTFIELD_ID)
...
class Fetch(base.Task):
    ...
    def initialize_episode(self, physics) -> None:
        ...
        # Initial ball state.
        physics.named.data.qpos['ball_root'][:2] = self.random.uniform(-spawn_radius, spawn_radius, size=(2,))
