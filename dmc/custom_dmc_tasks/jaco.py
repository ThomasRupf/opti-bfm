...
from dm_control.manipulation.shared import arenas, cameras, constants, observations, robots, workspaces
from dm_control.utils import rewards
from dm_env import specs

_ReachWorkspace = collections.namedtuple('_ReachWorkspace', ['target_bbox', 'tcp_bbox', 'arm_offset'])

# Ensures that the props are not touching the table before settling.
_PROP_Z_OFFSET = 0.001

_DUPLO_WORKSPACE = _ReachWorkspace(
    target_bbox=workspaces.BoundingBox(lower=(-0.1, -0.1, _PROP_Z_OFFSET), upper=(0.1, 0.1, _PROP_Z_OFFSET)),
...
def make(task_id, obs_type, seed, time_limit=None):
    ...
    time_limit = time_limit if time_limit is not None else _TIME_LIMIT
...
class MultiTaskReach(composer.Task):
    ...
    def __init__(self, task_id, arena, arm, hand, prop, obs_settings, workspace, control_timestep):
    ...
    def get_reward(self, physics):
        ...
        for target_pos in self._targets:
            ...
            reward = rewards.tolerance(distance, bounds=(0, _TARGET_RADIUS), margin=_TARGET_RADIUS)
            rews.append(reward)
...
def _reach(task_id, obs_settings, use_site):
    ...
    if use_site:
        ...
        prop = props.Duplo(
            observable_options=observations.make_options(obs_settings, observations.FREEPROP_OBSERVABLES)
        )
    task = MultiTaskReach(
        task_id,
        arena=arena,
