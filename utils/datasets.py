...
class GCDataset:
    ...
    def get_observations(self, idxs, key='observations'):
        ...
        if self.config['frame_stack'] is None or self.preprocess_frame_stack:
            ...
            return self.get_stacked_observations(idxs, key=key)

    def get_goal_observations(self, idxs):
        """Return the goal observations for the given indices."""
        key = 'oracle_reps' if 'oracle_reps' in self.dataset.keys() else 'observations'
        return self.get_observations(idxs, key=key)
